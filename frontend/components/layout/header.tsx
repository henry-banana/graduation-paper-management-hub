    ...
    <header className="bg-surface/80 backdrop-blur-xl sticky top-0 z-40 w-full border-b border-outline-variant/10 shadow-[0_4px_30px_rgba(11,28,48,0.04)]">
      <div className="flex items-center justify-between px-4 md:px-6 h-16">
        ...
        <div className="flex items-center gap-1">
          {/* Notification bell */}
          <button className="relative p-2 text-on-surface-variant hover:bg-primary/5 hover:text-primary rounded-full transition-colors active:scale-95">
          ...
          <div className="relative">
            ...
            >
              ...
              <div className="hidden md:block text-left">
                <p className="text-xs font-semibold text-on-surface leading-tight">{ROLE_LABELS[role] || role}</p>
                <p className="text-[10px] text-outline">Đang hoạt động</p>
              </div>
            ...
            {showDropdown && (
              <div className="absolute right-0 mt-2 w-52 bg-surface-container-lowest rounded-2xl shadow-xl border border-outline-variant/10 py-2 z-50">
                ...
                <div className="border-t border-outline-variant/10 mt-2 pt-2">
                  <Link href="/login" className="w-full flex items-center gap-3 px-4 py-2.5 text-sm text-error hover:bg-error-container/20 transition-colors rounded-b-2xl">
